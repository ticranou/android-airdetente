...
private fun HelpContent() {
    ...
    Bullet("Enregistreur de vol (ANLFDR) : conserve les dernières minutes de vol (GPS, accélération, inclinaison, altitude, baromètre). Double-tap = pause / reprise. Appui long = exporter dans Téléchargements : trace KML (Google Earth) ou GPX couvrant tout le vol (positions + altitude), ou log brut CSV des X dernières minutes (toutes les mesures). Récupérable en branchant l'appareil au PC. Il tourne en continu tant que l'application est active (service de vol), y compris quand vous passez sur une autre application ; il s'arrête au « Quitter ». Durée conservée et fréquence de sauvegarde réglables dans Réglages ▸ Cockpits. Pastille verte = paramètre disponible et enregistré, orange = capteur absent. Anneau orange = temps avant la prochaine sauvegarde disque.")
    Note(
