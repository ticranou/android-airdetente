...
private fun HelpContent() {
    ...
    Bullet("Enregistreur de vol (ANLFDR / NUMFDR) : conserve les dernières minutes de vol (GPS, accélération, inclinaison, altitude, baromètre). Double-tap = pause / reprise. Appui long = exporter dans Téléchargements : trace KML (Google Earth) ou GPX couvrant tout le vol (positions + altitude), ou log brut CSV des X dernières minutes (toutes les mesures). Récupérable en branchant l'appareil au PC. Il tourne en continu tant que l'application est active (service de vol), y compris quand vous passez sur une autre application ; il s'arrête au « Quitter ». Durée conservée et fréquence de sauvegarde réglables dans Réglages ▸ Cockpits. Pastille verte = paramètre disponible et enregistré, orange = capteur absent. Anneau / barre orange = temps avant la prochaine sauvegarde disque.")
    Bullet("Approche finale (CMNAPP, 3 lignes mini) : vue « tunnel » 3D vers la piste (highway-in-the-sky). Trois cadres emboîtés (vert 500 / jaune 1000 / orange 1500 ft au-dessus du tour de piste = altitude terrain + 1000 ft) fuient vers la piste. La silhouette d'ULM se décale selon l'écart d'axe (droite/gauche) et de plan (au-dessus/en dessous du plan à 3°) : verte dans les tolérances (±15 m d'axe, ±60 ft de plan), orange en dehors. Bandeau : IAS, altitude, vario, écart d'axe (m G/D) et écart au plan (ft / 3°). Cible = aérodrome le plus proche (AUTO) ; appui long = verrouiller un terrain et un QFU (ou revenir en AUTO). En v1 la cible est le point de référence de l'aérodrome (proxy du seuil) et le QFU vient de la fiche au pas de 10° : aide INDICATIVE, non certifiée, sans conscience du relief.")
    Note(
